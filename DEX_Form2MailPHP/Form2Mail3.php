...
//------------------------------------- BASIC CONFIGURATION !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   ** FROM y TO                                                                                    *
   *************************************************************************************************/
  define('DEX_SEND_EMAIL_FROM', 'd');
  define('DEX_FIX_DESTINATION_EMAIL', 'david@sitestoremember.com');

  /*************************************************************************************************
   ** Email subject, you can change it here                                                        *
   *************************************************************************************************/
  $emailsubject = 'Mailing List for Democracy counts';
   ...
   *************************************************************************************************/
  define('DEX_FROM_EMAIL_FIELD_NAME', 'email');
  define('DEX_SUBJECT_FIELD_NAME', '');

  /*************************************************************************************************
   ** SEND COPIES TO OTHER EMAIL ADDRESSES, SEMICOLON SEPARATED                                    *
   *************************************************************************************************/
  define('DEX_SEND_CC_TO','danielhwolf@gmail.com');

  /*************************************************************************************************
   ** EXCLUDED FIELDS                                                                              *
   *************************************************************************************************/
  $excluded_fields = array ();
  $excluded_fields[] = "submit";
$excluded_fields[] = "Submit";
$excluded_fields[] = "hdcaptcha";
;

  /*************************************************************************************************
   ** HTML FORMATTING AND SPECIAL CHARACTERS                                                       *
   *************************************************************************************************/
  define('USE_HTML_FORMATTING', true);
  define('SPECIAL_CHARACTERS', false);
  define('CHAR_SET', "iso-8859-1");

  /*************************************************************************************************
   ** Select to include or not, the additional information (country, ip, user-agent, ...)          *
   *************************************************************************************************/
  define ('DEX_INCLUDE_ADDITIONAL_INFORMATION', true);


//------------------------------------- SERVER CONFIGURATION !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  // ********************************************************************
  // CONNECTION SETTINGS FOR NON-STANDARD SERVERS
  // ********************************************************************
  define('MAILER_PRIMARY_METHOD', '');
...
//------------------------------------- COPY TO THE USER !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   *************************************************************************************************/
  define ('DEX_SEND_THANKYOU', false);
  define ('DEX_USER_EMAIL_FIELD_NAME', '');
  define ('DEX_INCLUDE_SUBMITTED_DATA', false);
  define ('DEX_THANKYOU_SUBJECT', '');
  define ('DEX_THANKYOU_MSG', '');


//------------------------------------- ADDITIONAL CONFIGURATION !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  // ********************************************************************
  // IF YOU WANT TO PUT SEPARATOR LINES INTO YOUR EMAIL
  // ********************************************************************
  ...
  define ("HDW_F2M_OK", "DEXok");
  define ("HDW_F2M_NO_OK", "DEXnook");

  // security verification
  if (!count($_GET) && !count($_POST) && !count($_FILES))
  {
      echo 'No information posted.';
      exit;
  }
  ...
  function send_mail($emailaddress, $fromaddress, $emailsubject, $body, $attachments=false)
  {
      ...
      if (MAILER_PRIMARY_METHOD == '')
          $mail->Mailer = "mail";
      else
          $mail->Mailer = MAILER_PRIMARY_METHOD;
      $result = $mail->Send();

      if (!$result) // line #174
      {
          if (MAILER_PRIMARY_METHOD == 'smtp')
              $mail->Mailer = "mail";
          else
              $mail->Mailer = "smtp";
          $result = $mail->Send();
  ...
  // To Email Address
  $emailaddress= DEX_FIX_DESTINATION_EMAIL;

  // From Email Address
  $fromaddress = DEX_SEND_EMAIL_FROM;
  if (DEX_FROM_EMAIL_FIELD_NAME != '' && getHDWForm2MailParam(DEX_FROM_EMAIL_FIELD_NAME) != '')
      $fromaddress = getHDWForm2MailParam(DEX_FROM_EMAIL_FIELD_NAME);

  // Message Subject
  if (DEX_SUBJECT_FIELD_NAME != '' && getHDWForm2MailParam(DEX_SUBJECT_FIELD_NAME) != '')
      $emailsubject = getHDWForm2MailParam(DEX_SUBJECT_FIELD_NAME);

  // Use relative paths for attachments
  $attachments = Array();
  ...
  foreach ($_GET as $item => $value)
    if ($item != HDW_F2M_OK && $item != HDW_F2M_NO_OK && notInThisArray($item))
  ...
  if (DEX_INCLUDE_ADDITIONAL_INFORMATION)
     $body .="<strong>ADDITIONAL INFORMATION</strong><br />"
           ."<strong>------------------------------------------------</strong><br />"
  ...
  // main email
  $main_result = send_mail($emailaddress, $fromaddress, $emailsubject, $body, $attachments);

  // send copies
  $copies = explode (";",DEX_SEND_CC_TO);
  foreach ($copies as $item)
      if (trim($item) != '' && strpos($item, "@") > 0)
          send_mail(trim($item), $fromaddress, $emailsubject, $body, $attachments);

  // send autoreply message to the user
  if (DEX_SEND_THANKYOU)
  {
      if (DEX_INCLUDE_SUBMITTED_DATA)
          send_mail(getHDWForm2MailParam(DEX_USER_EMAIL_FIELD_NAME), $fromaddress, DEX_THANKYOU_SUBJECT, DEX_THANKYOU_MSG ."<br /><br />". $body, $attachments);
      else
          send_mail(getHDWForm2MailParam(DEX_USER_EMAIL_FIELD_NAME), $fromaddress, DEX_THANKYOU_SUBJECT, DEX_THANKYOU_MSG, $attachments);
  }
  
  // redirect to the ok or error page
  if ($main_result)
...
class PHPMailer
{
    ...
    */
    ...
    function MailSend($header, $body) {
        ...
        if (!$rt)
        {
           ini_set("sendmail_from", "info@".$_SERVER["HTTP_HOST"]);
           $rt = @mail($to, $this->EncodeHeader($this->Subject), $body, $header); // if not successfull then test without ini_set
        }
...
function notInThisArray($value)
{
    ...
    for ($i=0;$i<count($excluded_fields);$i++)
        if ($excluded_fields[$i]==$value)
            return false;
    return true;
}
?>